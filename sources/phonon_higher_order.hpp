	...
	const std::vector<Term> eta_prime({
		Term(1, Coefficient("A_2", MomentumList({ 'K', 'L', 'P', 'Q' }), IndexWrapper{}, false, false),
			...
			std::vector<Operator>({
				...
				Operator(std::vector<MomentumSymbol>({ MomentumSymbol(1, 'K'), MomentumSymbol(1, 'P')}), Index::GeneralSpin_S, true),
				Operator('L', 1, false, Index::GeneralSpin_SPrime, true),
				Operator(std::vector<MomentumSymbol>({ MomentumSymbol(1, 'L'), MomentumSymbol(-1, 'Q') }), Index::GeneralSpin_SPrime, false),
				Operator(std::vector<MomentumSymbol>({ MomentumSymbol(1, 'K'), MomentumSymbol(1, 'Q') }), Index::GeneralSpin_S, false),
			})),
